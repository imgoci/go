...
// [github.com/imgoci/bigoci.WithHTTPClient] only when a client is injected. A
// nil HTTP client leaves bigoci to build its own default verified stack. The
// unverified external-transport option is never used.
//
...
// must never wrap these calls. A non-nil report callback converts each bigoci
// snapshot into that transfer's latest-absolute wire bytes and retries.
// Concurrent transfers are merged by the orchestrator, never here. A nil report
// skips conversion.
package multipart
