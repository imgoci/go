...
- **Dependency audit — rejection criteria:** the candidate (`gowebpki/jcs` v1.0.1; fallback: the cyberphone reference port, forked and fixed if necessary) is pinned only if it passes the full RFC 8785 vector corpus **plus** negatives: raw invalid UTF-8 in values and keys (our pre-gate covers the known `parseQuotedString` gap; the audit documents the division of labor); malformed `\u` escapes and lone/misordered surrogates; numbers outside JSON grammar or losing precision at parse (`1e400`, 2⁵³±1, `-0` per ES6); decoded-duplicate keys (ours to catch; audit verifies the transform's sorting doesn't mask them). The pin is by audit, not reputation (last release 2023); the `internal/jcs` wrapper keeps it swappable.
...
3. **JCS pin.** Decided at slice 1 by the §6.2 audit; fork-and-fix of the reference port is the floor if both candidates fail.
